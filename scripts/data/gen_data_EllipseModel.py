...
from data.myfit import EllipseModel, is_inlier, get_residuals
from gen_data import ellipse_data, get_bbox_limits, is_in_bbox, gen_uniform_noise
from estimation.estimation_results import get_ellipse_params
...
def gen_gauss_noise(data, model_class, model_params, residual_threshold, points_num, 
	...
	noise_list = np.empty((int(points_num), dim), dtype = float)
	i = 0
	trials = 0
	...
	while ((inliers_counter < inliers_num) or (outliers_counter < outliers_num)) and i < points_num:
		...
		x, y = noise_list[i]
		a,b,c,d,f,g = get_ellipse_params(model_params)
...
if __name__ == '__main__':
	...
	def generate_data(test_id):
		...
		try:
			...
			#residuals = get_residuals(gn_data, model_class, model_params)
			...
			inliers = residuals < 1.3*gn_params[4] # 1.3*sigma 99,7% inlier data
