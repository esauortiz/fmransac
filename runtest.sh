...
for i in $(seq $initial_batch $batches_num); do
	...
	python scripts/testcfg/gen_yaml_master_file.py ${save_path} batch_$i
	...
	python scripts/testcfg/gen_yaml_file.py batch_$i ${tests_num} ${save_path}
	...
	then
		python scripts/data/gen_data_${model}.py ${tests_num} ${save_path}
		echo 'Data generated'
	...
	python scripts/estimation/estimate_model.py ${tests_num} ${save_path} ransac # {ransac,msac,fun1,fun2,fun3,fun4}
	echo ''
	echo 'Tests finished. Generating results... '

	# Create results files
	#python scripts/estimation/estimation_results.py ${save_path} ${tests_num} ${model} ransac
